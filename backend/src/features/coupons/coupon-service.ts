  ...
  public static async createCoupon(input: CreateCouponInput, creatorUserId: string) {
    ...
    const coupon = await prisma.discountCoupon.create({
      data: {
        ...
        validFrom: input.validFrom && !isNaN(Date.parse(input.validFrom)) ? new Date(input.validFrom) : new Date(),
        validUntil: input.validUntil && !isNaN(Date.parse(input.validUntil)) ? new Date(input.validUntil) : null,

...
export interface FilingFloatingActionBarProps {
...
export const FilingFloatingActionBar: React.FC<FilingFloatingActionBarProps> = ({
  selectedCount,
  onAutoRoundRobin,
  onOpenAssignModal,
  onClearSelection,
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 animate-in fade-in slide-in-from-bottom-5 duration-200">
      <div className="flex items-center gap-3 px-5 py-3 rounded-xl bg-slate-900/95 backdrop-blur-md text-white border border-slate-700 shadow-2xl shadow-slate-950/40">
        <div className="flex items-center gap-2 pr-3 border-r border-slate-700">
          ...
          <div>
            <span className="text-xs font-bold text-white tracking-wide">
              {selectedCount} {selectedCount === 1 ? 'Return' : 'Returns'}
            </span>
        ...
        <Button
          ...
          disabled={isLoading}
          className="h-9 px-3.5 rounded-xl font-bold text-xs bg-[#16A34A] hover:bg-[#15803D] text-white flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
        >
        ...
        <Button
          ...
          disabled={isLoading}
          className="h-9 px-3.5 rounded-xl font-bold text-xs bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-600 flex items-center gap-1.5 transition-all cursor-pointer"
        >
